...
export const CharacterManager: React.FC<CharacterManagerProps> = ({ characters, isStudioBusy, setIsStudioBusy, setCharacters, onApiError }) => {
  ...
  const synthesizeVisualAnchor = async (char: Character) => {
    ...
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      // Use standard quality keywords for reliability in browser environments
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      {isFormOpen && (
        <div className="bg-[#100C0A] border-2 border-[#C6934B]/30 p-8 rounded-[3rem] shadow-2xl animate-in slide-in-from-top-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {characters.map(char => (
          <div key={char.id} className="bg-[#100C0A]/60 border border-[#3E2F28] rounded-[2.5rem] p-8 hover:border-[#C6934B]/40 transition-all group overflow-hidden relative">
            ...
            <p className="text-[11px] text-[#8C7A70] leading-relaxed line-clamp-3 mb-6 font-medium italic">"{char.description}"</p>
